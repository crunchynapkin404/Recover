...
/**
 * Intervals.icu structured-workout text syntax reference, ported verbatim
 * from the standalone intervals-icu-mcp server's workout_syntax.py
 * (WORKOUT_SYNTAX_SPEC), which itself adapts the spec from
 * intervals-icu-workout-parser by Marvin Nazari (MIT License).
 ...
 *
 * Standalone had this as an MCP resource; here it's a tool so it shows up
 * in tool listings without resource support.
 */
const WORKOUT_SYNTAX = `# Intervals.icu Workout Syntax Reference
...
async function execute(_args: z.infer<typeof parameters>, _ctx: ToolContext) {
  return { syntax: WORKOUT_SYNTAX };
}

export const getWorkoutSyntax: ToolDefinition<typeof parameters> = {
  name: "get_workout_syntax",
  description:
    "Reference for intervals.icu structured-workout text syntax (steps, targets, repeats) — read this before creating or editing workout events.",
  parameters,
  execute,
};
