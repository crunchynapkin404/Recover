...
export async function fetchAthleteContext(
  ...
  const latestWellness = wellness7.find(
  ...
  if (latest) {
    ...
    lines.push(`**Readiness:** ${latest.readiness}/100 (band: ${latest.band}) — ${latest.date}`);
    if (cs) {
      lines.push(
  ...
  if (wellness7.length > 1) {
    ...
    const rhrs = wellness7.filter((w) => w.restingHr != null).map((w) => w.restingHr!);
    if (hrvs.length > 0) {
      lines.push(
