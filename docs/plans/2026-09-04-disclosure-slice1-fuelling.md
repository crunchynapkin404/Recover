...
**Open risk carried from the spec.** `?sheet=fuelling` can be typed for a day with no session; Task 5 gates on `openDaySlot` so it renders nothing rather than an empty sheet. If a refusal message is wanted instead, that is a copy decision, not a structural one.
