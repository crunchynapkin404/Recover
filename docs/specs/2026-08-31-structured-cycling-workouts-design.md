...
| 5   | **The library, to 100+**  | Pure content. No code change — which is the test of whether slices 0–2 got the shape right. **SHIPPED IN TWO PARTS, and the first part was mistaken for the whole.** v0.127.0 authored 16 workouts to reach 46 and closed the strand under the name of this slice; 46 is not 100+, and nothing recorded the shortfall. Completed to **103 workouts across 22 families**, with `LIBRARY_TARGET` in `coverage-guard.test.ts` now asserting the number this row states. No code change, as predicted — the shape held |

Slices 0–2 are the design work; 3–4 are largely plumbing; 5 is authoring.

**How slice 5 was missed, recorded because the mechanism outlives it.** Nothing
here lied. v0.127.0 reported "two families at every covered duration" — true,
build-gated, and a real improvement — and that narrower metric silently took
the place of the goal this row states. Every check anyone ran was green,
because nothing checked scope. The same substitution had already happened once
in 2b.4, whose slices 7–9 became release-named work and left `globals.css`
pinned at 15px for thirteen releases under a comment promising it would move.
Two occurrences make it a pattern, so the fix is mechanical rather than a
resolution to be more careful: the number this row names is now asserted by a
test, and the family floor the library actually reached is a ratchet.
